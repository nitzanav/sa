...
etl: write_joint_analyst_recommendations
	...
	@psql "$$DATABASE_URL" -c "\copy (SELECT * FROM signals WHERE signal_name = 'yahoo-daily'  and analyst_projections_count > 2 and average_projected > 10 ORDER BY signal_percentile DESC) TO 'data/signals.csv' WITH CSV HEADER"
